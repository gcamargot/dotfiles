local el_ok, el = pcall(require, "el")
if not el_ok then
	return
end
...
local lsp_server = function(_, buffer)
	...
	local clients = (vim.lsp.get_clients and vim.lsp.get_clients({ bufnr = buffer.bufnr }))
		or (vim.lsp.get_active_clients and vim.lsp.get_active_clients({ bufnr = buffer.bufnr }))
		or {}
...
local diagnostics = function(_, buffer)
	local counts = vim.diagnostic.count and vim.diagnostic.count(buffer.bufnr)
	if not counts then
		return ""
	end
